 ...
 * Class Installation
 *
 * @property \App\Models\Installation $model
 *
 * @see http://sleepingowladmin.ru/docs/model_configuration_section
 */
class Installation extends Section implements Initializable
{
    public function initialize()
    {
        $this->title = 'Инсталяции';
    ...
    public function onEdit($id)
    {
        ...
        $tabs->setTabs(function () use($id) {
            ...
            $installation = $id ? \App\Models\Installation::find($id) : null;
            $elements = [
                ...
                AdminFormElement::columns()
                    ->addColumn([
                        $elements[] = AdminFormElement::text('values[link_text]', 'Текст')
                            ...
                            ->setDefaultValue($installation->values['link_text'] ?? '')
                    ])->addColumn([
                        $elements[] = AdminFormElement::text('values[link_path]', 'Адрес')
                            ->setValueSkipped(true)
                            ->setDefaultValue($installation->values['link_path'] ?? '')
                    ]),
